...
cmd({
    ...
    try {
        ...
        let apiRes;
        try {
            const response = await axios.get(`https://apiziaul.vercel.app/api/downloader/ytmp3?url=${encodeURIComponent(ytUrl)}`);
            apiRes = response.data;
        } catch (e) {
            const response = await axios.get(`https://apiziaul.vercel.app/api/downloader/ytplaymp3?query=${encodeURIComponent(ytUrl)}`);
            apiRes = response.data;
        }
